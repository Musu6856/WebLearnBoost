...
export function App() {
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      try {
        const [settings, history] = await Promise.all([loadInitialSettings(), loadHistory()]);
        if (!mounted) return;
        setState((current) => ({ ...current, settings, history }));
      } catch (error) {
        if (!mounted) return;
        setState((current) => ({
          ...current,
  ...
  const handleGenerateMap = async () => {
    ...
    if (!extractResult.ok) {
      setState((current) => ({ ...current, status: "failed", error: extractResult.error }));
      return;
    }
    ...
    const mapResult = await generateLearningMap(extractResult.data, state.settings);
    if (!mapResult.ok) {
      setState((current) => ({ ...current, status: "failed", error: mapResult.error }));
      return;
    }
  ...
  const handleStartTraining = async () => {
    ...
    setState((current) => ({ ...current, status: "generating-training", error: null }));
    const trainingResult = await buildTrainingPackage(state.extractedContent, state.learningMap, state.settings);

    if (!trainingResult.ok) {
      setState((current) => ({ ...current, status: "failed", error: trainingResult.error }));
      return;
    }

    const saveResult = await savePackage(trainingResult.data);
    if (!saveResult.ok) {
      setState((current) => ({
        ...current,
  ...
  const handleSaveSettings = async (settings: AppSettings) => {
    setState((current) => ({ ...current, settings, status: "saved", error: null }));
    const result = await saveSettings(settings);
    if (!result.ok) {
      setState((current) => ({ ...current, status: "failed", error: result.error }));
      return;
    }
  ...
  const handleExportMarkdown = async () => {
    ...
    const exportResult = await exportMarkdown(state.activePackage);
    if (!exportResult.ok) {
      setState((current) => ({ ...current, status: "failed", error: exportResult.error }));
      return;
    }

    const saveResult = await savePackage(exportResult.data);
    setState((current) => ({
      ...current,
  ...
  const handleLocateSourceQuote = async (quote: string) => {
    const result = await locateSourceQuote(quote);
    if (!result.ok) {
      setState((current) => ({ ...current, status: "failed", error: result.error }));
    }
  };
  ...
  const handleDeleteHistoryItem = async (learningPackage: LearningPackage) => {
    const result = await deletePackageVersion(learningPackage.id);
    if (!result.ok) {
      setState((current) => ({ ...current, status: "failed", error: result.error }));
      return;
    }
